          ...
          <div className="space-y-2">
            {reasons.map(([key, label]) => (
              <button
                ...
                className={`w-full flex items-center gap-3 rounded-lg border-2 px-4 py-3 text-left text-sm transition-colors ${selected === key
                    ? "border-destructive bg-destructive/5 text-foreground"
                    : "border-border hover:border-destructive/40 text-muted-foreground hover:text-foreground"
                  }`}
              >
                {/* Radio dot */}
                <span className={`h-4 w-4 shrink-0 rounded-full border-2 flex items-center justify-center ${selected === key ? "border-destructive" : "border-muted-foreground"
                  }`}>
                  {selected === key && (
