...
import type { EmployeeHistoryEntry } from "../../../api/assignment-history.api";
...
export function EmployeeHistoryModal( {employee , history, onClose,}: EmployeeHistoryModalProps) {
  const sortedHistory = [...history].sort( (a, b) => b.date.getTime() - a.date.getTime() );
  const today = new Date();

  const totalWorkload =
    sortedHistory.reduce( (total, assignment) => total + 
    calculateAssignmentWorkload( assignment.job, assignment.shiftGroup) , 0 );

  const currentFatigue = calculateCurrentFatigue( sortedHistory , today );

  const pastHistory = sortedHistory.filter(
    (assignment) => daysBetween( assignment.date , today ) >= 0 );

  const lastAssignment = pastHistory[0] ?? null;

  const daysSinceLastAssignment = lastAssignment ? daysBetween( lastAssignment.date, today ) : null;

  const jobCounts = new Map < string , { label: string; count: number;} >();

  for (const assignment of sortedHistory) {
    const current = jobCounts.get(assignment.job.key);
  ...
  const mostFrequentJob = Array.from(jobCounts.values()).sort( (a, b) => b.count - a.count )[0] ?? null;
